...
describe('Guard Security Tests', () => {
  ...
  it('should validate JWT token in guard - Test 1', async () => {
    const mockContext = {
      switchToHttp: () => ({
        getRequest: () => ({
          headers: {
  ...
  it('should enforce role-based access control - Test 1', async () => {
    const mockContext = {
      switchToHttp: () => ({
        getRequest: () => ({
          user: {
            userId: 'test-32d0co7c',
  ...
  it('should sanitize user input in guard context - Test 1', async () => {
    const maliciousRequest = {
      headers: {
        authorization: 'Bearer valid.jwt.token',
        'x-forwarded-for': '<script>alert("header xss32d0co7c")</script>',
