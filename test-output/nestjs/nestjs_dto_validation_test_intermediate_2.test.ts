...
describe('DTO Validation Security Tests', () => {
  it('should validate and sanitize CreateUserDto - Test 1', async () => {
    const maliciousData = {
      username: '<script>alert("dto xssg3if5w")</script>',
      email: 'invalid-emailg3if5w',
      password: '123', // too short
      bio: 'Normal bio with <img src=x onerror=alert("xss")> malicious content',
      website: 'javascript:alert("maliciousg3if5w")'
    };
  ...
  it('should sanitize HTML content in DTO - Test 1', async () => {
    const dataWithHtml = {
      username: 'validuserg3if5w',
      email: 'testg3if5w@example.com',  
  ...
  it('should validate nested object DTOs - Test 1', async () => {
    const nestedMaliciousData = {
      personalInfo: {
        firstName: '<script>alert("nested xssg3if5w")</script>',
      ...
      preferences: {
        theme: 'dark", "maliciousField": "injectedg3if5w',
        notifications: true
      }
    };
