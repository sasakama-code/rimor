...
describe('Git Hygiene - Issue #113, #118対応', () => {
  ...
  describe('TDD Red Phase: .jest-cacheの完全除外確認', () => {
    test('Git追跡されているファイル一覧に.jest-cacheファイルが含まれていない', () => {
      ...
      const jestCacheFiles = trackedFilesList.filter(file => file.startsWith('.jest-cache/'));

      expect(jestCacheFiles).toHaveLength(0);

      // より厳密な確認：jest-cacheという文字列を含むファイルがないこと
      const filesWithJestCache = trackedFilesList.filter(
        file => file.includes('jest-cache') && !file.includes('jest-cache-fix')
      );
    ...
    test('git statusで.jest-cacheディレクトリが無視されている', () => {
      ...
      if (fs.existsSync(path.join(projectRoot, '.jest-cache'))) {
        ...
        const jestCacheStatusLines = statusLines.filter(line => line.includes('.jest-cache'));
    ...
    test('git check-ignoreで.jest-cacheが無視対象として認識される', () => {
      try {
        ...
        const checkIgnoreResult = GitTestHelpers.execGitCommand(
          'git check-ignore .jest-cache/',
          {}
        ).trim();
  ...
  describe('Issue #118対応: 包括的キャッシュファイル検出（TDD-RED）', () => {
    ...
    test('Git追跡対象にJestトランスフォームキャッシュが含まれていない', () => {
      ...
      const jestTransformCacheFiles = trackedFilesList.filter(
        file =>
    ...
    test('潜在的PII露出リスクファイルの検出', () => {
      ...
      const actualRiskFiles = potentialPIIFiles.filter(
        file => !allowedExceptions.some(exception => file.includes(exception))
      );
  ...
  describe('Issue #118対応: CI/CD統合セキュリティ強化（フェーズ1.2-RED）', () => {
    test('CI環境でのキャッシュファイル検出強化', () => {
      ...
      try {
        ...
        if (ciCacheFiles.length > 0) {
          console.error(
            `[CI SECURITY RISK] 検出されたCI環境キャッシュファイル:\n${ciCacheFiles.join('\n')}`
          );
        }
    ...
    test('大容量リポジトリでのパフォーマンス検証', () => {
      ...
      console.log(
        `パフォーマンス情報: ${fileCount}ファイル処理に${executionTime}ms（${(executionTime / fileCount).toFixed(2)}ms/file）`
      );
    });
  });
