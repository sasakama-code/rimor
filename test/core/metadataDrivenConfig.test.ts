...
describe('MetadataDrivenConfigManager', () => {
  ...
  describe('推奨機能', () => {
    ...
    test('should recommend based on dependencies', () => {
      ...
      const recommendations = configManager.generateRecommendations(baseConfig);
      const dependentRecommendation = recommendations.find(
        rec => rec.pluginName === 'dependent-plugin'
      );
  ...
  describe('設定検証', () => {
    ...
    test('should detect unknown plugins', () => {
      ...
      expect(validation.errors.some(error => error.includes('unknown-plugin'))).toBe(true);
    });
    ...
    test('should detect missing dependencies', () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.errors.some(error => error.includes('missing-dependency'))).toBe(true);
    });
    ...
    test('should warn about performance issues', () => {
      ...
      expect(validation.warnings.some(warning => warning.includes('推定実行時間'))).toBe(true);
    });
  });
});
