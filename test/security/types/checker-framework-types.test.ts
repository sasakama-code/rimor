...
describe('Checker Framework Types', () => {
  ...
  describe('TypePropagation', () => {
    it('should propagate taint in binary operations', () => {
      ...
      const result = TypePropagation.binaryOperation(tainted, untainted, (a, b) => a + b);
    ...
    it('should preserve untainted in binary operations with both untainted', () => {
      ...
      const result = TypePropagation.binaryOperation(untainted1, untainted2, (a, b) => a + b);
    ...
    it('should propagate taint in method calls', () => {
      ...
      const result = TypePropagation.methodCall(taintedReceiver, 'concat', [untaintedArg], ((
        ...args: unknown[]
      ) => {
        // Simulate string concat method
        return taintedReceiver.__value + (args[0] as any);
      }) as any);
  ...
  describe('FlowSensitiveChecker', () => {
    ...
    it('should refine types based on conditions', () => {
      ...
      checker.refineType('value', v => !isNaN(Number(v)), '@Untainted', '@Tainted');
  ...
  describe('PolyTaintInstantiation', () => {
    it('should instantiate poly taint with any rule', () => {
      ...
      const result1 = PolyTaintInstantiation.instantiate(poly, ['@Tainted', '@Untainted']);
      expect(TypeGuards.isTainted(result1)).toBe(true);

      // No tainted arguments
      const result2 = PolyTaintInstantiation.instantiate(poly, ['@Untainted', '@Untainted']);
    ...
    it('should instantiate poly taint with all rule', () => {
      ...
      const result1 = PolyTaintInstantiation.instantiate(poly, ['@Tainted', '@Tainted']);
      expect(TypeGuards.isTainted(result1)).toBe(true);

      // Some untainted arguments
      const result2 = PolyTaintInstantiation.instantiate(poly, ['@Tainted', '@Untainted']);
      expect(TypeGuards.isUntainted(result2)).toBe(true);
    });
  });
  ...
  describe('Integration scenarios', () => {
    it('should handle complex taint flow', () => {
      ...
      // Force sanitization
      const sanitized = TypePromotion.sanitize(validated as TaintedType<string>, 'sql-escape');
      ...
      const finalQuery = TypePropagation.binaryOperation(query, sanitized, (a, b) => a + `'${b}'`);
    ...
    it('should handle polymorphic method correctly', () => {
      ...
      const processString = TypeConstructors.polyTaint((s: string) => s.toUpperCase(), [0], 'any');

      // With tainted input
      const taintedInput = TypeConstructors.tainted('secret', 'user');
      const result1 = PolyTaintInstantiation.instantiate(processString, ['@Tainted']);
      expect(TypeGuards.isTainted(result1)).toBe(true);

      // With untainted input
      const untaintedInput = TypeConstructors.untainted('safe');
      const result2 = PolyTaintInstantiation.instantiate(processString, ['@Untainted']);
      expect(TypeGuards.isUntainted(result2)).toBe(true);
    });
  });
});
