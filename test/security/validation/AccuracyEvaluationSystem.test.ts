...
describe('AccuracyEvaluationSystem', () => {
  ...
  describe('evaluateAccuracy', () => {
    ...
    it('推論メトリクスが正しく計算されること', async () => {
      const testCases: TestCase[] = [createMockTestCase('method1'), createMockTestCase('method2')];
  ...
  describe('monitorAccuracyInRealTime', () => {
    ...
    it('複数プロジェクトの監視ができること', async () => {
      ...
      await expect(evaluationSystem.monitorAccuracyInRealTime(projects)).resolves.not.toThrow();
    });
  });
