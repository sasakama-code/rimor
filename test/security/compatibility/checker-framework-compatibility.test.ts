...
describe('CheckerFrameworkCompatibility', () => {
  ...
  describe('Core compatibility features', () => {
    ...
    it('should support annotation file format (.jaif)', () => {
      ...
      expect(parsed.getAnnotation('com.example.MyClass', 'process', 'parameter', 0)).toBe(
        '@Tainted'
      );
      expect(parsed.getAnnotation('com.example.MyClass', 'process', 'return')).toBe('@Untainted');
    });
  });
  ...
  describe('AnnotationReader', () => {
    ...
    it('should read stub files', () => {
      ...
      expect(stubs.getMethodAnnotation('java.lang.String', 'toLowerCase')).toBe('@PolyTaint');
      expect(stubs.getMethodAnnotation('java.lang.String', 'length')).toBe('@Untainted');
    });
  });
  ...
  describe('TypeChecker', () => {
    ...
    it('should check method invocation compatibility', () => {
      ...
      // 正しい引数
      expect(typeChecker.checkMethodCall(methodSig, ['@Untainted', '@Untainted'])).toEqual({
        safe: true,
        violations: undefined,
      });

      // 間違った引数（@Taintedを@Untaintedパラメータに渡す）
      const result = typeChecker.checkMethodCall(methodSig, ['@Tainted', '@Untainted']);
    ...
    it('should support polymorphic type checking', () => {
      ...
      // @Tainted文脈
      const taintedInstance = typeChecker.instantiatePolyTaint(polyMethod, '@Tainted');
      expect(taintedInstance.qualifier).toBe('@Tainted');
      expect(taintedInstance.confidence).toBeGreaterThan(0);

      // @Untainted文脈
      const untaintedInstance = typeChecker.instantiatePolyTaint(polyMethod, '@Untainted');
