...
import {
  container as orchestratorContainer,
  TYPES as ORCHESTRATOR_TYPES,
} from '../../src/container';
...
describe('UnifiedSecurityAnalysisOrchestrator Coverage Integration', () => {
  ...
  describe('DIコンテナ統合', () => {
    it('UnifiedPluginManagerを正しく注入できる', () => {
      // ARRANGE
      const unifiedPluginManager = orchestratorContainer.get<UnifiedPluginManager>(
        ORCHESTRATOR_TYPES.UnifiedPluginManager
      );
    ...
    it('TestExistencePluginが品質プラグインとして登録されている', () => {
      ...
      const hasTestExistencePlugin = qualityPlugins.some(
        plugin => plugin.constructor.name === 'TestExistencePlugin'
      );
  ...
  describe('エラーハンドリング', () => {
    it('存在しないパスでもエラーを適切に処理する', async () => {
      ...
      // ACT & ASSERT
      await expect(orchestrator.analyzeTestDirectory(invalidPath)).rejects.toThrow();
    });
