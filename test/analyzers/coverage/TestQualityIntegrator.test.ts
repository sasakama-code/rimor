...
describe('TestQualityIntegrator', () => {
  ...
  describe('カバレッジ統合基本機能', () => {
    it('テストファイルが存在し、カバレッジが業界基準を満たす場合、高スコアを返すべき', () => {
      ...
      // Act
      const result = integrator.evaluateIntegratedQuality(
        testFile,
        highCoverage,
        mockProjectContext
      );
  ...
  describe('グレーディングシステム', () => {
    it('A評価(90-100点)の条件を満たすべき', () => {
      const excellentTestFile: TestFile = {
        path: '/test/project/src/example.test.ts',
        content:
          'describe("Example", () => { it("should work", () => { expect(true).toBe(true); }); it("should handle errors", () => { expect(() => { throw new Error(); }).toThrow(); }); });',
      };
