...
import { IFormattingStrategy, ReportFormat, UnifiedReport } from '../../src/reporting/core/types';
...
describe('UnifiedReportEngine', () => {
  ...
  describe('デコレーター戦略', () => {
    ...
    test('並列処理戦略の適用', async () => {
      ...
      const reports = await Promise.all(results.map(result => engine.generate(result)));
  ...
  describe('エラーハンドリング', () => {
    ...
    test('無効な入力データの処理', async () => {
      ...
      await expect(engine.generate(invalidData)).rejects.toThrow('Invalid analysis result');
    });
    ...
    test('フォーマッターエラーの処理', async () => {
      ...
      await expect(engine.generate(mockAnalysisResult)).rejects.toThrow('Formatting failed');
    });
  });
  ...
  describe('SOLID原則の遵守', () => {
    test('単一責任原則（SRP）', () => {
      ...
      expect(engineMethods.filter(m => !m.startsWith('_'))).toEqual(
