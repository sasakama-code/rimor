...
describe('AI JSON Command Coverage Tests', () => {
  ...
  beforeEach(() => {
    ...
    jest.clearAllMocks();
    // fsモックを実際の実装にリセット
    (fs.mkdtempSync as jest.Mock).mockImplementation(jest.requireActual('fs').mkdtempSync);
    (fs.existsSync as jest.Mock).mockImplementation(jest.requireActual('fs').existsSync);
    (fs.rmSync as jest.Mock).mockImplementation(jest.requireActual('fs').rmSync);
  ...
  describe('AI JSON 生成カバレッジ', () => {
    it('mapSeverityToRiskLevel関数の全パスをカバー', async () => {
      ...
      const commandWithCustomContainer = new AnalyzeCommandV8(
        testContainer,
        createMockCliSecurity()
      );
