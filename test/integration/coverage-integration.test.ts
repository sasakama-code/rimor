    ...
    it('低カバレッジファイルの検出統合テスト', async () => {
      ...
      if (lowCoverageFiles.length > 0) {
        ...
        for (let i = 1; i < lowCoverageFiles.length; i++) {
          expect(lowCoverageFiles[i].linesPct).toBeGreaterThanOrEqual(
            lowCoverageFiles[i - 1].linesPct
          );
        }
      }
    });
