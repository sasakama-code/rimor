...
describe('BaselineManager', () => {
  ...
  describe('基本的なベースライン操作', () => {
    ...
    it('ベースラインを正常に取得できる', async () => {
      const mockResults: BenchmarkResult[] = [createMockBenchmarkResult('TypeScript', 2.5, 0.95)];
  ...
  describe('ベースライン比較機能', () => {
    it('現在結果とベースラインを比較できる', async () => {
      ...
      const baselineResults: BenchmarkResult[] = [
        createMockBenchmarkResult('TypeScript', 3.0, 0.9),
      ...
      // Ant Designの劣化を確認
      const antdComparison = comparison.projectComparisons.find(
        p => p.projectName === 'Ant Design'
      );
    ...
    it('5ms/file目標達成率の比較ができる', async () => {
      const baselineResults: BenchmarkResult[] = [
        createMockBenchmarkResult('TypeScript', 3.0, 0.9, true), // 達成
  ...
  describe('自動ベースライン選択機能', () => {
    it('最適なベースラインを自動選択できる', async () => {
      ...
      const baseline1Id = await baselineManager.createBaseline(
        [createMockBenchmarkResult('TypeScript', 4.0, 0.8)],
        { name: 'old-baseline' }
      );
    ...
    it('プロジェクトごとの最適ベースラインを選択できる', async () => {
      await baselineManager.createBaseline([
        createMockBenchmarkResult('TypeScript', 2.0, 0.95),
        createMockBenchmarkResult('Ant Design', 5.0, 0.7),
      ]);

      await baselineManager.createBaseline([
        createMockBenchmarkResult('TypeScript', 4.0, 0.8),
        createMockBenchmarkResult('Ant Design', 3.0, 0.9),
      ]);
  ...
  describe('統計的分析機能', () => {
    ...
    it('複数ベースライン間の傾向分析ができる', async () => {
      ...
      const trendAnalysis = await baselineManager.analyzeTrends([
        baseline1Id,
        baseline2Id,
        baseline3Id,
      ]);
  ...
  describe('ベースライン管理機能', () => {
    ...
    it('ベースライン一覧を取得できる', async () => {
      await baselineManager.createBaseline([createMockBenchmarkResult('TypeScript', 2.5, 0.95)], {
        name: 'baseline-1',
      });
      await baselineManager.createBaseline([createMockBenchmarkResult('Ant Design', 3.1, 0.88)], {
        name: 'baseline-2',
      });
  ...
  describe('エラーハンドリングとDefensive Programming', () => {
    ...
    it('空の結果配列に対して適切にエラーハンドリングする', async () => {
      await expect(baselineManager.createBaseline([])).rejects.toThrow(
        '空の結果配列からベースラインを作成することはできません'
      );
    });
