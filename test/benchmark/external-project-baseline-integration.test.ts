...
describe('BaselineManager統合テスト', () => {
  ...
  describe('パフォーマンス改善検知機能', () => {
    it('性能改善を検知できる', async () => {
      ...
      // パフォーマンス改善が記録されている
      const projectComparison = comparison.projectComparisons.find(p =>
        p.projectName.includes('performance-improvement-test')
      );
    ...
    it('性能劣化を検知できる', async () => {
      ...
      // パフォーマンス劣化が記録されている
      const projectComparison = comparison.projectComparisons.find(p =>
        p.projectName.includes('performance-degradation-test')
      );
      expect(projectComparison).toBeDefined();
    });
    ...
    it('5ms/file目標達成状況の変化を追跡できる', async () => {
      ...
      // 改善または継続のいずれかに分類される
      expect(improvements.improved.length + improvements.maintained.length).toBeGreaterThan(0);
    });
  });
...
`;
    await fs.writeFile(path.join(projectDir, 'src', `test${i}.ts`), fileContent);
  }
