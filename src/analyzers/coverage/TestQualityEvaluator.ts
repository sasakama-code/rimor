...
export class TestQualityEvaluator {
  ...
  calculateWeightedScore(coverage: CoverageSummary): number {
    ...
    const statementScore = this.calculateMetricScore(
      ...
      this.defaultThresholds.statements
    );
    const functionScore = this.calculateMetricScore(
      coverage.functions.pct,
      this.defaultThresholds.functions
    );
    const branchScore = this.calculateMetricScore(
      coverage.branches.pct,
      this.defaultThresholds.branches
    );
  ...
  identifyQualityIssues(coverage: CoverageSummary): string[] {
    ...
    if (coverage.lines.pct < this.defaultThresholds.lines) {
      issues.push(
        `行カバレッジが基準値${this.defaultThresholds.lines}%を下回っています (現在: ${coverage.lines.pct}%)`
      );
    }
    ...
    if (coverage.statements.pct < this.defaultThresholds.statements) {
      issues.push(
        `ステートメントカバレッジが基準値${this.defaultThresholds.statements}%を下回っています (現在: ${coverage.statements.pct}%)`
      );
    }
    ...
    if (coverage.functions.pct < this.defaultThresholds.functions) {
      issues.push(
        `関数カバレッジが基準値${this.defaultThresholds.functions}%を下回っています (現在: ${coverage.functions.pct}%)`
      );
    }
    ...
    if (coverage.branches.pct < this.defaultThresholds.branches) {
      ...
      issues.push(
        `[${severity}] 分岐カバレッジが基準値${this.defaultThresholds.branches}%を大幅に下回っています (現在: ${coverage.branches.pct}%)`
      );
    }
    ...
    if (minCoverage < 30) {
      issues.push(
        `[CRITICAL] 一部のメトリクスで極めて低いカバレッジが検出されました (最低: ${minCoverage}%)`
      );
    }
