...
export class CoverageAnalyzer {
   ...
   * 詳細カバレッジファイルを読み込む
   */
  async loadCoverageFinal(
    coverageDir: string
  ): Promise<Record<string, CoverageFileDetails> | null> {
    try {
  ...
  isCoverageAdequate(coverage: CoverageSummary, thresholds?: CoverageThresholds): boolean {
    ...
    return (
      coverage.lines.pct >= limits.lines &&
  ...
  private calculateFileCoverage(fileData: CoverageFileDetails): CoverageSummary {
    ...
    return {
      lines: {
        ...
        pct:
          statements.length > 0
            ? Math.round((coveredStatements / statements.length) * 100 * 100) / 100
            : 100,
      },
      statements: {
        total: statements.length,
      ...
      functions: {
        ...
        pct:
          functions.length > 0
            ? Math.round((coveredFunctions / functions.length) * 100 * 100) / 100
            : 100,
      },
      branches: {
        ...
        pct:
          branches.length > 0
            ? Math.round((coveredBranches / branches.length) * 100 * 100) / 100
            : 100,
      },
    };
  }
