...
export class MetricsCalculator {
  ...
  calculateTestability(
    ...
    sourceFiles.forEach(sourceFile => {
      ...
      const hasTest = testFiles.some(
        testFile => testFile.includes(baseName) && testFile.includes('test')
      );
    ...
    const coverage =
      sourceFiles.length > 0 ? (sourceFiles.length - untestedFiles.length) / sourceFiles.length : 0;
  ...
  calculateDocumentationCoverage(code: string): number {
    ...
    const jsdocPattern =
      /\/\*\*[\s\S]*?\*\/\s*(?:function|class|interface|export\s+(?:function|class))/g;
  ...
  private async calculateTestabilityMetrics(
    ...
    // モック可能性の計算（簡易版）
    const mockability =
      testableClasses > 0 ? testableClasses / (testableClasses + untestableClasses) : 0;
  ...
  private async calculateDocumentationMetrics(
    ...
    codeContents.forEach(content => {
      // JSDocコメントを持つ関数/クラスをカウント
      const jsdocFunctions = (
        content.match(/\/\*\*[\s\S]*?\*\/\s*(?:function|async\s+function)/g) || []
      ).length;
