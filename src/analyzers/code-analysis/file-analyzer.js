...
class FileAnalyzer {
     ...
     * 関連ファイルの検索
     */
    async findRelatedFiles(currentFilePath, projectPath, options) {
        const relatedFiles = [];
        try {
            ...
            for (const importObj of imports) {
                ...
                if (resolvedPath) {
                    ...
                    for (const possiblePath of possiblePaths) {
                        const safePath = pathSecurity_1.PathSecurity.safeResolve(possiblePath, projectPath, 'findRelatedFiles');
     ...
     * テストファイルの検索
     */
    async findTestFiles(filePath, projectPath) {
        const testFiles = [];
        ...
        for (const pattern of testPatterns) {
            ...
            const safeTestPath = pathSecurity_1.PathSecurity.safeResolve(testPath, projectPath, 'findTestFiles');
        ...
        const safeTestsDir = pathSecurity_1.PathSecurity.safeResolve(testsDir, projectPath, 'findTestFiles');
     ...
     * 同階層ファイルの検索
     */
    async findSiblingFiles(filePath, projectPath) {
        const siblingFiles = [];
        const dirName = path.dirname(filePath);
        const currentFileName = path.basename(filePath);
        try {
            const safeDirName = pathSecurity_1.PathSecurity.safeResolve(dirName, projectPath, 'findSiblingFiles');
            if (!safeDirName)
                return siblingFiles;
            ...
            for (const file of files) {
                ...
                if (['.ts', '.js', '.tsx', '.jsx'].includes(ext)) {
                    ...
                    const safeSiblingPath = pathSecurity_1.PathSecurity.safeResolve(siblingPath, projectPath, 'findSiblingFiles');
                    if (safeSiblingPath) {
     ...
     * プロジェクト内の全ソースファイルを検索
     */
    findAllSourceFiles(projectPath) {
        const sourceFiles = [];
        try {
            const safeProjectPath = pathSecurity_1.PathSecurity.safeResolve(projectPath, process.cwd(), 'findAllSourceFiles');
            if (!safeProjectPath)
                return sourceFiles;
     ...
     * ファイル間の依存関係分析
     */
    async analyzeDependencies(filePath, projectPath) {
        const dependencies = [];
        const dependents = [];
        try {
     ...
     * 関連ファイルの分析
     */
    async analyzeRelatedFile(filePath, originFilePath) {
        try {
     ...
     * ファイル構造の分析
     */
    async analyzeFileStructure(filePath, projectPath) {
        // ファイル構造の分析を実装
        const safeFilePath = pathSecurity_1.PathSecurity.safeResolve(filePath, projectPath, 'analyzeFileStructure');
     ...
     * 関数名の抽出
     */
    async extractFunctionNames(content, language) {
     ...
     * 可能なファイルパスの生成
     */
    generatePossiblePaths(basePath, projectPath) {
     ...
     * ディレクトリの深さ制限チェック
     */
    isWithinDepthLimit(filePath, projectPath, maxDepth = 10) {
        const relativePath = path.relative(projectPath, filePath);
     ...
     * ファイルサイズ制限チェック
     */
    isWithinSizeLimit(filePath, maxSize = 1024 * 1024) {
        try {
            const stats = fs.statSync(filePath);
     ...
     * 循環依存の検出
     */
    async detectCircularDependencies(filePath, projectPath) {
        const visited = new Set();
        const recursionStack = new Set();
        const cycles = [];
        const detectCycle = async (currentFile, path) => {
