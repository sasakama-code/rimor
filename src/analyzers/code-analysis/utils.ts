...
export class CodeContextUtils {
  ...
  calculateConfidence(fileContent: string, language: string): number {
    ...
    if (hasInterfaces) confidence += 0.05;
    if (hasTypes) confidence += 0.05;
    ...
    if (hasImports) confidence += 0.05;
    if (hasExports) confidence += 0.05;
  ...
  calculateReadabilityScore(fileContent: string): number {
    ...
    // 命名規則の一貫性（変数名）
    const variableNames =
      fileContent.match(/(?:const|let|var)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g) || [];
   ...
   * 依存関係の循環検出用のヘルパー
   */
  buildDependencyGraph(files: Array<{ path: string; imports: string[] }>): Map<string, string[]> {
