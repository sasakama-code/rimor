...
class ScopeAnalyzer {
     ...
     * スコープコンテキストの分析
     */
    async analyzeScopeContext(fileContent, targetLine) {
        const scopes = [];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (hasOpenBrace && (scopeType === 'function' || scopeType === 'block' || scopeType === 'if' || scopeType === 'loop')) {
                ...
                const newScope = {
                    type: scopeType === 'if' || scopeType === 'loop' ? 'block' : scopeType,
     ...
     * コンテンツからスコープ階層を分析
     */
    async analyzeScopeHierarchy(content, language) {
     ...
     */
    extractScopes(content, language) {
        // 同期的に処理を実行（後方互換性のため）
        const scopes = [];
     ...
     * 特定の行のスコープを特定
     */
    findScopeAtLine(scopes, targetLine) {
        // 対象行を含むすべてのスコープを見つける
        const containingScopes = scopes.filter(scope => targetLine >= scope.startLine && targetLine <= scope.endLine);
     ...
     * スコープ階層の取得
     */
    getScopeHierarchy(scopes, targetLine) {
        const hierarchy = [];
        // 対象行を含むすべてのスコープを収集
        const containingScopes = scopes.filter(scope => targetLine >= scope.startLine && targetLine <= scope.endLine);
        // レベル順にソート
     ...
     * スコープ内の変数一覧取得
     */
    getVariablesInScope(scope) {
        return scope.variables;
    }
    /**
     * スコープ内の関数一覧取得
     */
    getFunctionsInScope(scope) {
     ...
     * ネストレベルの計算
     */
    calculateNestingLevel(line, currentLevel) {
     ...
     * キャプチャされた変数の検出
     */
    findCapturedVariables(lines, startLine) {
        const capturedVars = [];
     ...
     * クロージャーの終了行を検出
     */
    findClosureEnd(lines, startLine) {
        let braceCount = 0;
        let foundStart = false;
        for (let i = startLine; i < lines.length; i++) {
            const line = lines[i];
     ...
     */
    detectVariableShadowing(scopes) {
        const shadowings = [];
