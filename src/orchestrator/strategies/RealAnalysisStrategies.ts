...
export class RealTaintAnalysisStrategy implements ITaintAnalysisStrategy {
  ...
  async analyze(
    ...
    try {
      console.log(
        `🔍 [RealTaintAnalysisStrategy] 分析開始: ${targetPath}${options?.benchmarkMode ? ' (ベンチマークモード)' : ''}`
      );
      ...
      throw new Error(
        `Taint分析に失敗しました: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
...
export class RealGapDetectionStrategy implements IGapDetectionStrategy {
  ...
  async detect(
    ...
    try {
      ...
      throw new Error(
        `Gap検出に失敗しました: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
...
export class RealNistEvaluationStrategy implements INistEvaluationStrategy {
  ...
  async evaluate(gapResult: GapAnalysisResult): Promise<NistEvaluationResult> {
    try {
      ...
      throw new Error(
        `NIST評価に失敗しました: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
