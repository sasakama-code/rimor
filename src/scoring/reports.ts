...
import { ProjectScore, FileScore, DirectoryScore, DimensionScore, GradeType } from './types';
...
export class ReportGenerator {
  ...
  private calculateDimensionAverages(fileScores: FileScore[]): SummaryReport['dimensionScores'] {
    ...
    const totals = fileScores.reduce(
      (acc, file) => {
  ...
  private generateRecommendations(projectScore: ProjectScore): string[] {
        ...
        switch (dimension) {
          case 'completeness':
            recommendations.push(
              'テストカバレッジの向上を図り、未テストのコードパスを特定して下さい'
            );
            break;
          case 'correctness':
            recommendations.push(
              'アサーションの精度を向上させ、より厳密なテストケースを作成して下さい'
            );
            break;
          ...
          case 'performance':
            recommendations.push(
              'テスト実行時間の最適化とパフォーマンステストの追加を検討して下さい'
            );
            break;
          case 'security':
    ...
    if (lowGradeRatio > 0.3) {
      recommendations.push(
        '30%以上のファイルが低品質です。優先的に改善対象を決めて段階的に取り組んで下さい'
      );
    }
  ...
  private generateMetadata(
    ...
    const averageFileScore =
      projectScore.fileScores.length > 0
        ? projectScore.fileScores.reduce((sum, file) => sum + file.overallScore, 0) /
          projectScore.fileScores.length
        : 0;
    ...
    Object.entries(fileScore.dimensions).forEach(([dimensionName, dimension]) => {
      if (dimension.score < 60) {
        switch (dimensionName) {
          case 'completeness':
            suggestions.push(
              'このファイルの完全性を向上させるため、テストカバレッジの向上を推奨します'
            );
            break;
          case 'correctness':
            suggestions.push(
              '正確性を向上させるため、アサーションをより具体的かつ網羅的にして下さい'
            );
            break;
          case 'maintainability':
  ...
  private generatePredictions(historicalScores: HistoricalScore[], currentScore: number) {
    ...
    const trend =
      recentScores.slice(1).reduce((sum, score, i) => sum + (score - recentScores[i]), 0) /
    ...
    switch (trend) {
      case 'improving':
        ...
        if (improvementRate > 10) {
          recommendations.push(
            '急速な改善が見られます。変更内容を他のプロジェクトにも適用することを検討して下さい'
          );
        }
        break;
      case 'declining':
