...
export type { ConfigGenerationOptions, PluginRecommendation };
...
export class MetadataDrivenConfigManager {
  ...
  private selectPluginsByPreset(
    ...
    switch (preset) {
      ...
      case 'recommended':
        return allPlugins
          ...
          .sort((a, b) => {
            ...
            return (
              (categoryPriority[b.category as string] || 0) -
              (categoryPriority[a.category as string] || 0)
            );
          })
      ...
      case 'performance':
        return allPlugins
          .filter(
            p => p.performance?.estimatedTimePerFile && p.performance.estimatedTimePerFile <= 15
          )
          .sort(
            (a, b) =>
              (a.performance?.estimatedTimePerFile || 0) -
              (b.performance?.estimatedTimePerFile || 0)
          )
          .map(p => p.name);
   ...
   * プラグイン優先度の計算
   */
  private calculatePluginPriority(
    metadata: PluginMetadata,
    options: ConfigGenerationOptions
  ): number {
    let priority = 100;
    ...
    if (
      metadata.performance?.estimatedTimePerFile &&
      metadata.performance.estimatedTimePerFile <= 10
    )
      priority += 15;
  ...
  private estimateExecutionTime(pluginNames: string[]): number {
    ...
    return pluginNames.reduce((total, pluginName) => {
      ...
      return (
        total +
        (metadata?.performance?.estimatedTimePerFile
          ? metadata.performance.estimatedTimePerFile * assumedFileCount
          : 0)
      );
    }, 0);
  }
  ...
  generateRecommendations(currentConfig: RimorConfig): PluginRecommendation[] {
    ...
    for (const metadata of allPlugins) {
      ...
      if (metadata.category === 'core' && !currentPlugins.has(metadata.name)) {
        const corePluginCount = Array.from(currentPlugins).filter(
          name => pluginMetadataRegistry.get(name)?.category === 'core'
        ).length;
      ...
      if (
        metadata.performance?.estimatedTimePerFile &&
        metadata.performance.estimatedTimePerFile <= 5 &&
        metadata.performance?.memoryUsage === 'low'
      ) {
        recommendations.push({
          pluginName: metadata.name,
          reason: '高速で軽量な分析を提供',
  ...
  optimizeConfig(
    ...
    if (targetMetrics.maxExecutionTime) {
      ...
      if (currentEstimate > targetMetrics.maxExecutionTime) {
        ...
        const sortedBySpeed = pluginEntries
          ...
          .sort(
            (a, b) =>
              (a.metadata?.performance?.estimatedTimePerFile || 0) -
              (b.metadata?.performance?.estimatedTimePerFile || 0)
          );
    ...
    if (targetMetrics.maxMemoryUsage) {
      ...
      Object.keys(optimizedConfig.plugins).forEach(pluginName => {
        const metadata = pluginMetadataRegistry.get(pluginName);
        if (
          metadata?.performance?.memoryUsage &&
          memoryPriority[metadata.performance.memoryUsage] < maxAllowed
        ) {
  ...
  validateConfig(config: RimorConfig): {
    ...
    for (const pluginName of Object.keys(config.plugins)) {
      ...
      for (const dep of metadata.dependencies || []) {
        if (!dep.optional && !config.plugins[dep.pluginName]) {
          errors.push(
            `プラグイン「${pluginName}」は「${dep.pluginName}」に依存していますが、有効になっていません`
          );
        }
      }
    }
    ...
    if (totalEstimatedTime > 60000) {
      // 1分以上
      warnings.push(
        `推定実行時間が${Math.round(totalEstimatedTime / 1000)}秒です。パフォーマンスを考慮してプラグインを減らすことを検討してください`
      );
    }

    // 競合検出
    const conflicts = pluginMetadataRegistry.detectConflicts();
    const enabledConflicts = conflicts.filter(
      conflict => config.plugins[conflict.plugin1] && config.plugins[conflict.plugin2]
    );

    for (const conflict of enabledConflicts) {
      warnings.push(
        `プラグイン「${conflict.plugin1}」と「${conflict.plugin2}」で競合が検出されました: ${conflict.reason}`
      );
    }
