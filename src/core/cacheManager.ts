...
export class CacheManager {
   ...
   * 複数ファイルの分析結果を一括取得
   */
  async getBatch(requests: Array<{ filePath: string; pluginName: string }>): Promise<
    Array<{
      filePath: string;
      pluginName: string;
      results: Issue[] | null;
  ...
  async setBatch(
    ...
    await Promise.all(
      entries.map(entry => this.set(entry.filePath, entry.pluginName, entry.results))
    );
  }
  ...
  async preWarm(filePaths: string[], pluginNames: string[]): Promise<void> {
    console.log(
      `キャッシュプリウォーミング開始: ${filePaths.length}ファイル × ${pluginNames.length}プラグイン`
    );
  ...
  private isFileModified(entry: CacheEntry, fileStats: fsSync.Stats): boolean {
    return entry.lastModified !== fileStats.mtimeMs || entry.fileSize !== fileStats.size;
  }
  ...
  private async loadCacheFromDisk(): Promise<void> {
    try {
      ...
      // 検証済みなので型アサーションが安全
      const validatedData = cacheData as {
      ...
      errorHandler.handleError(error, undefined, '', { cacheFilePath: this.cacheFilePath }, true);
    }
  }
  ...
  private validateCacheEntry(key: string, entry: unknown): boolean {
    ...
    // ファイルパスの検証
    if (
      typeof entryObj.filePath !== 'string' ||
      entryObj.filePath.includes('..') ||
      entryObj.filePath.length > 500
    ) {
      return false;
    }
    ...
    // 数値フィールドの検証
    if (
      typeof entryObj.fileSize !== 'number' ||
      entryObj.fileSize < 0 ||
      entryObj.fileSize > 100 * 1024 * 1024
    ) {
      return false;
    }
  ...
  async saveToDisk(filePath?: string): Promise<void> {
    ...
    try {
      ...
      errorHandler.handleError(error, undefined, '', { cacheFilePath: targetPath }, true);
    }
  }
