...
import type { PluginParameter, PluginDependency, PluginMetadata } from './types/config-types';

// 型定義を再エクスポート（後方互換性のため）
export type { PluginParameter, PluginDependency, PluginMetadata };
...
export class PluginMetadataRegistry {
  ...
  getStatistics(): {
    ...
    const categoryBreakdown = allPlugins.reduce(
      (acc, plugin) => {
    ...
    const totalParameters = allPlugins.reduce(
      (sum, plugin) => sum + (plugin.parameters?.length || 0),
      0
    );
    const totalDependencies = allPlugins.reduce(
      (sum, plugin) => sum + (plugin.dependencies?.length || 0),
      0
    );
