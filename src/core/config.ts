...
export type { PluginConfig, PluginMetadata, RimorConfig, ConfigGenerationOptions };

export class ConfigLoader {
  private static readonly CONFIG_FILENAMES = ['.rimorrc.json', '.rimorrc', 'rimor.config.json'];
  ...
  async loadConfig(startDir: string): Promise<RimorConfig> {
    ...
    if (configPath) {
      try {
        ...
        const validationResult = await this.configSecurity.loadAndValidateConfig(
          configPath,
          projectRoot
        );
        ...
        if (
          ...
          console.warn(
            `セキュリティ警告（修正済み）: ${validationResult.securityIssues.join(', ')}`
          );
        }
  ...
  async loadOrGenerateConfig(
    ...
    console.log(
      `メタデータ駆動設定を生成しました（プラグイン数: ${Object.keys(generatedConfig.plugins).length}）`
    );
  ...
  private async discoverAvailablePlugins(): Promise<void> {
    try {
        ...
        if (pluginFiles.includes(legacy.file)) {
          this.availablePlugins.push({
            ...
            defaultConfig: {
              enabled: true,
              excludeFiles:
                legacy.name === 'test-existence'
                  ? ['index.ts', 'index.js', 'types.ts', 'types.js', 'config.ts', 'config.js']
                  : undefined,
            },
          });
        }
      ...
      const corePlugins = [
        {
          file: 'core/AssertionQualityPlugin.ts',
          name: 'assertion-quality',
          displayName: 'Assertion Quality',
          description: '',
        },
        {
          file: 'core/TestCompletenessPlugin.ts',
          name: 'test-completeness',
          displayName: 'Test Completeness',
          description: '',
        },
        {
          file: 'core/TestStructurePlugin.ts',
          name: 'test-structure',
      ...
      errorHandler.handleError(error, undefined, '', undefined, true);
...
const mergeOptionalProperty = <T>(base?: T, override?: T): T | undefined => {
