...
export class CachedAnalyzer {
  ...
  async showCacheInfo(): Promise<void> {
    ...
    if (info.entries.length > 0) {
      ...
      info.entries.slice(0, 5).forEach(entry => {
        ...
        console.log(
          `    ${relativePath} (${this.formatBytes(entry.fileSize)}, ${entry.pluginCount}プラグイン)`
        );
      });
    }
  }
  ...
  private async analyzeFileWithCache(
    ...
    for (const plugin of plugins) {
      ...
      try {
        ...
        // パフォーマンス監視記録
        if (this.options.enablePerformanceMonitoring) {
          this.performanceMonitor.endPluginExecution(
            plugin.name,
            filePath,
            pluginStartTime,
            issues
  ...
  getCacheStats(): CachedAnalysisResult['cacheStats'] & {
    ...
    const totalHits =
      this.lastCacheStats.cacheHits > 0 ? this.lastCacheStats.cacheHits : stats.hits;
    const totalMisses =
      this.lastCacheStats.cacheMisses > 0 ? this.lastCacheStats.cacheMisses : stats.misses;
    const totalFilesFromCache =
      this.lastCacheStats.filesFromCache > 0 ? this.lastCacheStats.filesFromCache : totalHits;
    const totalFilesAnalyzed =
      this.lastCacheStats.filesAnalyzed > 0 ? this.lastCacheStats.filesAnalyzed : totalMisses;
