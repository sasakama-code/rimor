...
export class StatisticalDomainAnalyzer {
  ...
  constructor(config?: Partial<DomainAnalysisConfig>) {
    ...
    this.config = {
      ...
      supportedExtensions:
        config?.supportedExtensions !== undefined
  ...
  public async collectSourceFiles(): Promise<string[]> {
    // 配列の存在を確認
    if (
      !Array.isArray(this.config.supportedExtensions) ||
      this.config.supportedExtensions.length === 0
    ) {
      return [];
    }
  ...
  private extractTokensFromNode(node: unknown, tokens: string[]): void {
    ...
    // 識別子、クラス名、関数名などを抽出
    if (
      nodeObj.type === 'identifier' ||
      nodeObj.type === 'property_identifier' ||
      nodeObj.type === 'type_identifier'
    ) {
      if (typeof nodeObj.text === 'string' && nodeObj.text.length > 2) {
        // 2文字以上のトークンのみ
  ...
  public async analyze(): Promise<DomainAnalysisResult> {
    ...
    const filtered = this.filterLowFrequencyTokens(frequencies, this.config.minKeywordFrequency);
