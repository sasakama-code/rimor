...
export class DictionaryBootstrap {
  ...
  async runBootstrap(): Promise<void> {
    try {
      console.log('🚀 Rimor ドメイン辞書セットアップウィザードへようこそ！\n');
      ...
      console.log('\n✅ ブートストラップが完了しました！');
      console.log('次のコマンドで辞書を使用できます:');
      console.log('  npm run analyze -- --dictionary');
      console.log('  rimor dictionary list');
      console.log('  rimor dictionary validate\n');
  ...
  private async selectInitializationMethod(): Promise<'auto' | 'manual' | 'import'> {
    console.log('\n🔧 辞書の初期化方法を選択してください:');
    console.log('1. 自動生成 (既存コードから知識を抽出)');
    console.log('2. 手動設定 (対話的に辞書を作成)');
    console.log('3. インポート (既存の辞書ファイルを使用)');
    ...
    switch (choice) {
      ...
      default: 
        console.log('無効な選択です。自動生成を使用します。');
        return 'auto';
    }
  }
  ...
  private async autoGenerateDictionary(projectInfo: any): Promise<DomainDictionary> {
    ...
    try {
      ...
      console.log(`📍 検出された設定ファイル:`, linterConfigs);
      ...
      console.log(`✅ 辞書を自動生成しました (用語: ${extractedKnowledge.terms.length}, ルール: ${extractedKnowledge.rules.length})`);
      
      return dictionaryManager.getDictionary();
    } catch (error) {
      console.warn('⚠️  自動生成に失敗しました。基本辞書を作成します。');
  ...
  private async manualDictionarySetup(projectInfo: any): Promise<DomainDictionary> {
    ...
    while (addMoreTerms && termCount < 10) {
      ...
      try {
        ...
        console.log(`✅ 用語 "${termName}" を追加しました`);
        
        const continueAdding = await this.askQuestion('他の用語も追加しますか？ (y/N): ');
        if (continueAdding.toLowerCase() !== 'y') break;
        
      } catch (error) {
        console.warn(`⚠️  用語 "${termName}" の追加に失敗しました: ${error}`);
      }
    }

    console.log(`✅ 手動辞書セットアップが完了しました (${termCount}個の用語)`);
    return dictionaryManager.getDictionary();
  }
  ...
  private async importExistingDictionary(projectInfo: any): Promise<DomainDictionary> {
    ...
    try {
      ...
      console.log(`✅ 辞書をインポートしました: ${dictionary.terms.length}個の用語, ${dictionary.businessRules.length}個のルール`);
      return dictionary;
      
    } catch (error) {
      console.warn(`⚠️  インポートに失敗しました: ${error}`);
      console.log('基本辞書を作成します...');
      return this.createBasicDictionary(projectInfo);
    }
  }
  ...
  private createBasicDictionary(projectInfo: any): DomainDictionary {
    ...
    for (const termData of basicTerms) {
      try {
        ...
        console.warn(`基本用語 "${termData.term}" の追加に失敗しました`);
      }
    }
  ...
  private async generateConfiguration(projectInfo: any, dictionary: DomainDictionary): Promise<void> {
    ...
    console.log(`✅ 辞書ファイルを保存しました: ${dictionaryPath}`);
  }
  ...
  private async runInitialValidation(dictionary: DomainDictionary): Promise<void> {
    ...
    try {
      ...
      console.log(`📊 検証結果:`);
      console.log(`  - 用語数: ${termsCount}`);
      console.log(`  - ルール数: ${rulesCount}`);
      console.log(`  - ドメイン: ${dictionary.domain}`);
      console.log(`  - バージョン: ${dictionary.version}`);
      
      if (termsCount === 0) {
        console.warn('⚠️  辞書に用語が含まれていません。後で追加することをお勧めします。');
      }
      
      if (termsCount > 0) {
        console.log('✅ 辞書の基本検証が完了しました');
      }
      
    } catch (error) {
      console.warn('⚠️  検証中に問題が発生しました:', error);
    }
  }
