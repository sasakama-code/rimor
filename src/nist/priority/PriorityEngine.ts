...
export class PriorityEngine {
  ...
  calculatePriority(request: RiskPriorityRequest): RiskPriorityResult {
    ...
    const technicalComplexityScore = this.calculateTechnicalComplexityScore(
      request.technicalComplexity
    );
    const scopeScore = this.calculateScopeScore(request.affectedComponents, request.dependencies);

    let finalScore =
      baseRiskScore * businessImpactScore * (1 + scopeScore / 100) * (2 - technicalComplexityScore);
