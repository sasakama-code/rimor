...
export class LoggingMonitoringFailuresPlugin extends OWASPBasePlugin {
  ...
  isApplicable(context: ProjectContext): boolean {
    ...
    return deps.some((dep: string) => loggingLibs.includes(dep) || monitoringTools.includes(dep));
  }
  ...
  evaluateQuality(patterns: DetectionResult[]): QualityScore {
    const loggingTests = patterns.filter(
      p => p.patternId && p.patternId.startsWith('logging-') && p.metadata?.hasTest
    );
    const missingTests = patterns.filter(p => p.patternId && p.patternId.startsWith('missing-'));
    ...
    // 総合スコア計算
    const overall =
      missingTests.length > 0 ? Math.max(0.3, coverage * 0.5) : Math.min(0.9, 0.7 + coverage * 0.2);

    // セキュリティスコア計算
    const hasSecurityLogging = loggingTests.some(p => p.metadata?.testType === 'security-logging');
    const hasAuditTrail = loggingTests.some(p => p.metadata?.testType === 'audit-trail');
  ...
  async validateSecurityTests(testFile: TestFile): Promise<OWASPTestResult> {
    ...
    if (!loggingTests.some(p => p.metadata?.testType === 'security-logging')) {
      recommendations.push(
        '認証失敗、アクセス拒否などのセキュリティイベントログテストを追加してください'
      );
    }
    if (!loggingTests.some(p => p.metadata?.testType === 'audit-trail')) {
