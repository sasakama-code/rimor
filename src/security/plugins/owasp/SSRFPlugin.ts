...
export class SSRFPlugin extends OWASPBasePlugin {
  ...
  isApplicable(context: ProjectContext): boolean {
    ...
    // URLパーサーライブラリの確認
    const urlLibs = ['url-parse', 'whatwg-url', 'url', 'querystring'];
  ...
  evaluateQuality(patterns: DetectionResult[]): QualityScore {
    const ssrfTests = patterns.filter(
      p => p.patternId && p.patternId.startsWith('ssrf-') && p.metadata?.hasTest
    );
    ...
    // セキュリティスコア計算
    const hasSSRFPrevention = ssrfTests.some(p => p.metadata?.testType === 'ssrf-prevention');
    const hasUrlValidation = ssrfTests.some(p => p.metadata?.testType === 'url-validation');
  ...
  async validateSecurityTests(testFile: TestFile): Promise<OWASPTestResult> {
    ...
    if (!ssrfTests.some(p => p.metadata?.testType === 'ssrf-prevention')) {
      recommendations.push(
        '内部ネットワークやメタデータサービスへのアクセスを防ぐテストを追加してください'
      );
    }
    if (!ssrfTests.some(p => p.metadata?.testType === 'url-validation')) {
