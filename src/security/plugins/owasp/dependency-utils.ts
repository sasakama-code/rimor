 ...
 */
export function hasDependency(context: ProjectContext, dependencyName: string): boolean {
 ...
 */
export function hasAnyDependency(context: ProjectContext, dependencyNames: string[]): boolean {
 ...
 */
export function hasDependencyPattern(context: ProjectContext, patterns: string[]): boolean {
  const deps = context.dependencies;

  if (!deps) return false;

  if (Array.isArray(deps)) {
    return deps.some(dep => patterns.some(pattern => dep.includes(pattern)));
  } else if (typeof deps === 'object') {
    const depNames = Object.keys(deps);
    return depNames.some(dep => patterns.some(pattern => dep.includes(pattern)));
  }
