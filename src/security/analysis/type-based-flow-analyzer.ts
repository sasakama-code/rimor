...
export class TypeBasedFlowAnalyzer {
   ...
   * @returns 型ベース解析結果
   */
  async analyzeTypeBasedFlow(
    sourceCode: string,
    fileName: string
  ): Promise<TypeBasedAnalysisResult> {
   ...
   */
  private async setupAdvancedTypeScriptProgram(
    sourceCode: string,
    fileName: string
  ): Promise<void> {
  ...
  private extractTypeAnnotation(
  ...
  ): TypeBasedTaintInfo['typeAnnotation'] | undefined {
    ...
    const isUntaintedAnnotation =
      typeText.includes('@Untainted') || typeText.includes('Untainted<');
    ...
    if (ts.isParameter(node)) {
      ...
      if (
        parentFunction &&
        (ts.isFunctionDeclaration(parentFunction) ||
          ts.isFunctionExpression(parentFunction) ||
          ts.isArrowFunction(parentFunction))
      ) {
   ...
   * 型アノテーションから汚染状態を決定
   */
  private determineTaintStatusFromAnnotation(
    annotation?: TypeBasedTaintInfo['typeAnnotation']
  ): 'tainted' | 'untainted' | 'unknown' {
  ...
  private isRelatedToVariable(constraint: TypeConstraint, variableName: string): boolean {
    ...
    return (
      constraint.sourceVariable === cleanVarName ||
    ...
    const dfs = (
      ...
      for (const constraint of constraints) {
        ...
        if (this.matchesConstraintSource(constraint, currentVar)) {
          ...
          this.matchesConstraintTarget(constraint, currentVar)
        ) {
   ...
   * プロパティアクセス経由のパスを探索
   */
  private findPropertyAccessPath(
    sourceVariable: string,
    sink: TaintSink,
    constraints: TypeConstraint[]
  ): TypeConstraint[] {
  ...
  private isVariableRelatedToSink(variableName: string, sink: TaintSink): boolean {
    ...
    const functionNamePattern = new RegExp(
      `^${sink.dangerousFunction.functionName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\[`
    );
    ...
    // 部分一致（より緩い条件）
    return (
      sink.dangerousFunction.functionName.includes(cleanVarName) ||
  ...
  private isVariableUsedInSink(variableName: string, sink: TaintSink): boolean {
    ...
    const parameterPattern = new RegExp(
      `${cleanVariableName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\[param\\d+\\]`
    );
    ...
    return (
      isUsedInArgs ||
      isUsedInFunction ||
      isParameterConstraint ||
      isUsedInFunctionName ||
      isPropertyAccess ||
      isPartialMatch ||
      isFunctionChainUsage
    );
  }
  ...
  private checkFunctionChainUsage(variableName: string, sink: TaintSink): boolean {
    ...
    for (let i = sinkLine - 1; i >= 0; i--) {
      ...
      if (assignmentMatch) {
        ...
        if (
          this.isVariableInDataFlow(newVariable, sinkLine, lines) &&
   ...
   * 関数がSinkと関連するかチェック
   */
  private isFunctionRelatedToSink(
   ...
   */
  private isVariableInExpression(
    targetVariable: string,
    expression: string,
    currentLine: number,
    lines: string[]
    ...
    for (let i = currentLine - 1; i >= 0; i--) {
      ...
      const derivationMatch = line.match(
        new RegExp(
          `(const|let|var)\\s+${expression.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*=\\s*(.+)`
        )
      );
      if (derivationMatch) {
  ...
  private detectSanitizationPatterns(source: TaintSource, sink: TaintSink): number {
    ...
    // 2. 型変換による安全化
    const typeConversionPatterns = [/parseInt\(/g, /parseFloat\(/g, /Number\(/g, /\.toString\(\)/g];
  ...
  private validateTypeConstraints(
    constraintPath: TypeConstraint[]
  ): TypeBasedDataFlowPath['typeValidation'] {
    ...
    for (const constraint of constraintPath) {
      ...
      if (sourceInfo && targetInfo && !isSanitizationFlow) {
        ...
        if (sourceInfo.taintStatus === 'tainted' && targetInfo.taintStatus === 'untainted') {
          ...
          violatedConstraints.push(
            `型制約違反: tainted変数 ${sourceVarKey} から untainted変数 ${targetVarKey} への代入`
          );
        }
      }
      ...
      if (sourceInfo?.typeAnnotation && targetInfo?.typeAnnotation && !isSanitizationFlow) {
        ...
        if (sourceTaintType === 'tainted' && targetTaintType === 'untainted') {
          isTypeSafe = false;
          violatedConstraints.push(
            `JSDoc型制約違反: @tainted変数 ${sourceVarKey} から @untainted変数 ${targetVarKey} への代入`
          );
        }
      }
    }
    ...
    if (hasTypeAnnotations && constraintPath.length > 0) {
      ...
      for (const constraint of constraintPath) {
        ...
        if (
          sourceInfo?.typeAnnotation?.customTaintType === 'tainted' &&
   ...
   * 型ベースサマリーの計算
   */
  private calculateTypeBasedSummary(
    paths: TypeBasedDataFlowPath[],
    constraints: TypeConstraint[]
  ): TypeBasedAnalysisResult['summary'] {
    ...
    if (this.taintInfoMap.size > 0) {
      ...
      for (const [varName, typeInfo] of this.taintInfoMap) {
        ...
        if (
          typeInfo.typeAnnotation?.customTaintType === 'untainted' ||
    ...
    for (const constraint of constraints) {
      ...
      if (
        sourceInfo?.taintStatus === 'tainted' &&
        targetInfo?.typeAnnotation?.customTaintType === 'untainted'
      ) {
  ...
  private createBasicDataFlowPath(source: TaintSource, sink: TaintSink): TypeConstraint[] {
    ...
    for (let i = source.location.line - 1; i < sink.location.line && i < lines.length; i++) {
      ...
      const functionCallAssignmentMatch = line.match(
  ...
  private generateIntermediateSteps(source: TaintSource, sink: TaintSink): TypeConstraint[] {
    ...
    for (let i = source.location.line - 1; i < sink.location.line && i < lines.length; i++) {
      ...
      const functionAssignmentMatch = line.match(
        /(const|let|var)\s+(\w+)\s*=\s*(\w+)\s*\(\s*([^)]*)\s*\)/
      );
      ...
      if (finalCallMatch) {
        ...
        if (
          argument === currentVariable &&
          this.isFunctionRelatedToSink(
            functionName,
            sink.dangerousFunction.functionName,
            sourceCode
          )
        ) {
          steps.push({
   ...
   * 短いパスを拡張
   */
  private enhanceShortPath(
    source: TaintSource,
    sink: TaintSink,
    existingPath: TypeConstraint[]
  ): TypeConstraint[] {
   ...
   * 追加の中間ステップを作成
   */
  private createAdditionalIntermediateSteps(
    source: TaintSource,
    sink: TaintSink,
    existingPath: TypeConstraint[]
  ): TypeConstraint[] {
