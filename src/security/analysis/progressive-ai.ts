...
export class TypeAwareProgressiveAI {
  ...
  private generatePhaseSummary(methods: TestMethod[], phase: number): TypeSummary {
    ...
    const hasSafeMethod = methods.some(
      m => (m.name && m.name.includes('safe')) || (m.content && m.content.includes('validate'))
    );
    const hasUnsafeMethod = methods.some(
      m =>
    ...
    const hasCriticalElements = methods.some(
      m =>
        (m.name &&
          (m.name.includes('Critical') ||
            m.name.includes('auth') ||
            m.name.includes('password'))) ||
        (m.content &&
          (m.content.includes('password') ||
            m.content.includes('auth') ||
            m.content.includes('hash')))
    );
    ...
    const hasSecurityContent = methods.some(
      m =>
        (m.content &&
          (m.content.includes('user') ||
            m.content.includes('input') ||
            m.content.includes('validate'))) ||
        (m.name &&
          (m.name.includes('Input') || m.name.includes('Valid') || m.name.includes('Auth')))
    );
  ...
  private identifyCriticalPaths(flowGraph: FlowGraph): CriticalPathDetail[] {
    return (flowGraph.paths || [])
      .filter(
        path =>
          compareTaintLevels(path.taintLevel || 'untainted', TaintLevel.LIKELY_TAINTED) >= 0 &&
          !path.passedThroughSanitizer
      )
      .map(path => ({
        pathId: path.id || 'unknown',
  ...
  private generateOptimizationSuggestions(flowGraph: FlowGraph): OptimizationSuggestion[] {
    ...
    // セキュリティ最適化
    const vulnerablePaths = (flowGraph.paths || []).filter(
      p =>
        compareTaintLevels(p.taintLevel || 'untainted', TaintLevel.LIKELY_TAINTED) >= 0 &&
        !p.passedThroughSanitizer
    );
  ...
  private generateDetailedFlowGraphs(): DetailedFlowGraph[] {
    return Array.from(this.flowGraphs.entries()).map(([methodName, graph]) => ({
      ...
      vulnerablePaths: (graph.paths || []).filter(
        p =>
          compareTaintLevels(p.taintLevel || 'untainted', TaintLevel.LIKELY_TAINTED) >= 0 &&
          !p.passedThroughSanitizer
      ).length,
  ...
  private assessPathRisk(path: FlowPath): string {
    if (
      path.taintLevel &&
      path.taintLevel >= TaintLevel.LIKELY_TAINTED &&
      !path.passedThroughSanitizer
    ) {
  ...
  private categorizeIssue(
    issue: SecurityIssue
  ): 'injection' | 'xss' | 'auth' | 'data-leak' | 'other' {
