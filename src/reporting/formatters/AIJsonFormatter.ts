...
export class AIJsonFormatter extends BaseFormatter {
  ...
  protected doFormat(
    ...
    let filteredRisks = this.filterRisksByLevel(
      aiKeyRisks,
      options?.includeRiskLevels as string[] | undefined
    );
  ...
  private formatActionObject(
    action: string | { type?: string; description?: string; example?: string } | undefined
  ): { type: string; description: string; example?: string } {
    if (!action) {
      return {
        type: 'review',
