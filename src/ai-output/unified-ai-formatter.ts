...
import { AnalysisResult, ProjectContext, Issue } from '../core/types';
...
import {
  ...
  AIJsonOutput,
  UnifiedAIFormatterOptions as TypesFormatterOptions,
} from './types';
...
function isUnifiedAnalysisResult(result: unknown): result is UnifiedAnalysisResult {
  return (
    typeof result === 'object' && result !== null && 'summary' in result && 'aiKeyRisks' in result
  );
}
...
function isAnalysisResult(result: unknown): result is AnalysisResult {
  return (
    typeof result === 'object' && result !== null && 'issues' in result && 'filePath' in result
  );
}
...
interface FormatterStrategy {
  name: string;
  format(
    result: UnifiedAnalysisResult | AnalysisResult,
    options?: UnifiedAIFormatterOptions
  ): UnifiedAIOutput | AIJsonOutput;
  formatAsync?(
    result: UnifiedAnalysisResult | AnalysisResult,
    options?: UnifiedAIFormatterOptions
  ): Promise<UnifiedAIOutput | AIJsonOutput>;
}
...
export class UnifiedAIFormatter extends UnifiedAIFormatterBase {
  ...
  private initializeDefaultStrategies(): void {
    ...
    this.strategies.set('parallel', {
      name: 'parallel',
      format: (result: UnifiedAnalysisResult | AnalysisResult) => this.baseFormat(result),
      formatAsync: async (result: UnifiedAnalysisResult | AnalysisResult) =>
        this.parallelFormatAsync(result),
    });
  }
   ...
   */
  private async parallelFormatAsync(
    result: UnifiedAnalysisResult | AnalysisResult,
    options?: UnifiedAIFormatterOptions
  ): Promise<UnifiedAIOutput | AIJsonOutput> {
   ...
   */
  async formatBatch(
    results: (UnifiedAnalysisResult | AnalysisResult)[]
  ): Promise<(UnifiedAIOutput | AIJsonOutput)[]> {
  ...
  formatAsAIJson(
    ...
    options: Partial<TypesFormatterOptions> = {}
  ): AIJsonOutput {
    // Validate input
    if (!unifiedResult) {
    ...
    const prioritizedRisks = filteredRisks.slice(0, 10);
  ...
  private formatAnalysisResult(
    ...
    context?: ProjectContext | UnifiedAIFormatterOptions
  ): UnifiedAIOutput {
  ...
  private extractContext(context: ProjectContext): Record<string, unknown> {
    ...
    return {
      ...
      configuration: {
        ...
        hasTypeScript:
          context.languages?.includes('typescript') || context.language === 'typescript',
      },
    };
  }
  ...
  protected generateRecommendations(result: AnalysisResult): string[] {
    ...
    if (issuesByCategory['test-quality']?.length > 10) {
      recommendations.push('Implement test quality standards and automated quality checks');
    }

    // Performance recommendations
    if (issuesByCategory['performance']?.length > 5) {
      recommendations.push('Review and optimize test performance to reduce execution time');
    }

    // Security recommendations
    if (issuesByCategory['security']?.length > 0) {
      recommendations.push('Address security-related test gaps immediately');
    }

    // Documentation recommendations
    if (issuesByCategory['documentation']?.length > 15) {
      recommendations.push('Improve test documentation for better maintainability');
    }

    // Best practice recommendations
    if (issuesByCategory['best-practice']?.length > 20) {
      recommendations.push('Conduct team training on testing best practices');
    }
