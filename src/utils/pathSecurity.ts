...
export class PathSecurity {
    ...
    try {
      ...
      if (context === 'security-test') {
        ...
        if (!isValid) {
          errorHandler.handleError(
            new Error(
              `セキュリティテスト: 不正なファイルパス '${normalizedFilePath}' がプロジェクト範囲外にアクセスしようとしました`
            ),
      ...
      // 疑似テスト環境バイパス攻撃対策
        ...
        if (!isValid) {
          errorHandler.handleError(
            new Error(
              `不正なファイルパス '${normalizedFilePath}' がプロジェクト範囲外にアクセスしようとしました`
            ),
            ErrorType.PERMISSION_DENIED,
   ...
   */
  static safeResolveWithExtensions(
    basePath: string,
    extensions: string[],
    projectPath: string
  ): string | null {
    // Issue #161修正: basePath が相対なら projectPath を起点に絶対化（CWD基準の誤解決を防止）
    const resolvedBase = path.isAbsolute(basePath) ? basePath : path.resolve(projectPath, basePath);
  ...
  private static maskUserPaths(filePath: string): string {
    ...
    maskedPath = maskedPath.replace(homePathPattern, '[HOME]/');
  ...
  private static shortenByProjectName(maskedPath: string, projectName: string): string {
    ...
    // プロジェクト名パターンによる短縮処理
    let resultPath = this.replaceProjectNamePatterns(
   ...
   */
  private static normalizePathSeparators(path: string): {
   ...
   * @returns 置換処理されたパス
   */
  private static replaceProjectNamePatterns(
    normalizedPath: string,
    escapedProjectName: string,
    projectName: string
  ): string {
    ...
    const patterns = [
      ...
      new RegExp(`\\[HOME\\]/[^/]*/${escapedProjectName}/(.*)`, 'g'),
