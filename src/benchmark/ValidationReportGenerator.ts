...
export class ValidationReportGenerator {
  ...
  private calculateOverallEffectiveness(results: ExternalBenchmarkResult[]) {
    ...
    results.forEach(result => {
      ...
      const intentScore = Math.min(
        25,
        ua.intentExtraction.totalIntents * 1.5 + ua.intentExtraction.confidenceScore * 15
      );

      // ギャップ分析の有効性（0-25点）
      const gapScore = Math.min(
        25,
        ua.gapAnalysis.totalGaps * 2 + ua.gapAnalysis.implementationCoverage * 15
      );

      // NIST評価の有効性（0-25点）
      const nistScore = Math.min(
        25,
        ua.nistEvaluation.complianceScore * 20 + Math.min(5, ua.nistEvaluation.improvementProposals)
      );
  ...
  private evaluateTaintTyperEffectiveness(results: ExternalBenchmarkResult[]): FeatureEffectiveness {
    ...
    const avgAccuracy =
      validResults.reduce(
        (sum, r) => sum + r.unifiedAnalysis!.securityAnalysis.estimatedAccuracy,
        0
      ) / validResults.length;

    const avgCoverage =
      validResults.reduce((sum, r) => sum + r.unifiedAnalysis!.securityAnalysis.coverageRate, 0) /
      validResults.length;

    const effectivenessScore = Math.min(
      100,
      (avgDetections >= 5 ? 30 : avgDetections * 6) + avgAccuracy * 40 + avgCoverage * 30
    );
  ...
  private evaluateIntentExtractionEffectiveness(results: ExternalBenchmarkResult[]): FeatureEffectiveness {
    ...
    const avgIntents =
      validResults.reduce((sum, r) => sum + r.unifiedAnalysis!.intentExtraction.totalIntents, 0) /
      validResults.length;

    const avgConfidence =
      validResults.reduce(
        (sum, r) => sum + r.unifiedAnalysis!.intentExtraction.confidenceScore,
        0
      ) / validResults.length;

    const avgSuccessRate =
      validResults.reduce((sum, r) => sum + r.unifiedAnalysis!.intentExtraction.successRate, 0) /
      validResults.length;

    const effectivenessScore = Math.min(
      100,
      (avgIntents >= 10 ? 40 : avgIntents * 4) + avgConfidence * 30 + avgSuccessRate * 30
    );
  ...
  private evaluateGapAnalysisEffectiveness(results: ExternalBenchmarkResult[]): FeatureEffectiveness {
    ...
    const avgGaps =
      validResults.reduce((sum, r) => sum + r.unifiedAnalysis!.gapAnalysis.totalGaps, 0) /
      validResults.length;

    const avgCoverage =
      validResults.reduce(
        (sum, r) => sum + r.unifiedAnalysis!.gapAnalysis.implementationCoverage,
        0
      ) / validResults.length;

    const effectivenessScore = Math.min(
      100,
      (avgGaps >= 3 ? 50 : avgGaps * 16.7) + avgCoverage * 50
    );
  ...
  private evaluateNistEvaluationEffectiveness(results: ExternalBenchmarkResult[]): FeatureEffectiveness {
    ...
    const avgComplianceScore =
      validResults.reduce((sum, r) => sum + r.unifiedAnalysis!.nistEvaluation.complianceScore, 0) /
      validResults.length;

    const avgProposals =
      validResults.reduce(
        (sum, r) => sum + r.unifiedAnalysis!.nistEvaluation.improvementProposals,
        0
      ) / validResults.length;

    const effectivenessScore = Math.min(
      100,
      avgComplianceScore * 70 + Math.min(30, avgProposals * 5)
    );
  ...
  private assessPracticality(results: ExternalBenchmarkResult[]) {
    ...
    validResults.forEach(r => {
      ...
      Object.entries(r.unifiedAnalysis.securityAnalysis.severityDistribution).forEach(
        ([severity, count]) => {
...
${report.detectionCases.highValueCases
  .map(
    case_ =>
...
`;
  ...
  private analyzeSizeCorrelation(results: ExternalBenchmarkResult[]): CorrelationData {
    ...
    const sizeDetectionPairs = results
      ...
      .map(r => ({
        ...
        detections: Object.values(r.unifiedAnalysis!.securityAnalysis.detectionsByType).reduce(
          (a, b) => a + b,
          0
        ),
      }));
  ...
  private analyzeComplexityCorrelation(results: ExternalBenchmarkResult[]): CorrelationData {
    ...
    return {
      ...
      dataPoints: complexityDetectionPairs.length,
      trendDescription:
        correlation > 0.3 ? '複雑度と検出数に正の相関' : '複雑度との明確な相関は見られない',
    };
  }
  ...
  private analyzeProjectTypeCorrelation(results: ExternalBenchmarkResult[]): Record<string, number> {
    ...
    results.forEach(r => {
      ...
      const detections = Object.values(r.unifiedAnalysis.securityAnalysis.detectionsByType).reduce(
        (a, b) => a + b,
        0
      );
  ...
  private calculateIndustryGap(results: ExternalBenchmarkResult[]): number {
    ...
    const avgDetectionRate =
      validResults.reduce((sum, r) => {
        const detections = Object.values(
          r.unifiedAnalysis!.securityAnalysis.detectionsByType
        ).reduce((a, b) => a + b, 0);
