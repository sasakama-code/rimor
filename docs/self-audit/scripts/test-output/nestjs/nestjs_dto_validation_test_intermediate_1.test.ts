...
describe('DTO Validation Security Tests', () => {
  it('should validate and sanitize CreateUserDto - Test 0', async () => {
    const maliciousData = {
      username: '<script>alert("dto xssg7i92d")</script>',
      email: 'invalid-emailg7i92d',
      password: '123', // too short
      bio: 'Normal bio with <img src=x onerror=alert("xss")> malicious content',
      website: 'javascript:alert("maliciousg7i92d")'
    };
  ...
  it('should sanitize HTML content in DTO - Test 0', async () => {
    const dataWithHtml = {
      username: 'validuserg7i92d',
      email: 'testg7i92d@example.com',  
  ...
  it('should validate nested object DTOs - Test 0', async () => {
    const nestedMaliciousData = {
      personalInfo: {
        firstName: '<script>alert("nested xssg7i92d")</script>',
      ...
      preferences: {
        theme: 'dark", "maliciousField": "injectedg7i92d',
        notifications: true
      }
    };
